...
public class ImportAnimatedImages : Modification
{
    ...
    public class ImageSelectorPatch
    {
    ...
    public class SkipOverwritePatch
    {
        [HarmonyPostfix]
    ...
    public class SpritesheetPatch
    {
        public static void Prefix(ref string spritePath)
        {
            ...
            if (img == null || (img is not GIFFile && !img.IsAnimated))
                return;
            ...
            tab + $"\"clips\": [\n" +
            tab + tab + "{\"name\": \"neutral\", \"loop\": \"yes\", " + $"\"frames\": [{frames}], \"fps\": {(totalDelay == 0d ? 0f : (float)(img.FrameCount / totalDelay))} }}\n" +
            tab + "],\n" +
