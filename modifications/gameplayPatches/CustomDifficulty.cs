...
public class CustomDifficulty : Modification
{
    ...
    public class DifficultyPatch
    {
        ...
        public static bool HitPrefix(ref float __result, RDPlayer player)
        {
            ...
            {
                __result = MarginMultiplier(HitMargin.Value / 1000f);
                return false;
            }
            return true;
        }
        ...
        public static bool ReleasePrefix(ref float __result, RDPlayer player)
        {
            ...
            || (player == RDPlayer.P2 && P2Enabled.Value))
            {
                __result = MarginMultiplier(Mathf.Clamp(HitMargin.Value / 1000f, 0.08f, 0.4f));
                return false;
            }
            return true;
        }

        public static float MarginMultiplier(float input)
        {
            float ret = input;
    ...
    public class ButtonHitStripPatch
    {
        [HarmonyPostfix]
        ...
        {
            if (P1Enabled.Value)
                scnGame.p1DefibMode = DefibModeViaHitMargins();
            if (P2Enabled.Value)
                scnGame.p2DefibMode = DefibModeViaHitMargins();
        }
        ...
        public static DefibMode DefibModeViaHitMargins()
        {
            float hitmar = HitMargin.Value;
            if (hitmar <= 40f)
